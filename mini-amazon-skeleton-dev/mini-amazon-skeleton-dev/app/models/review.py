...
class ProductReview:
    ...
    @staticmethod
    def get_by_product(product_id, user_id=None):
        """Get all reviews for a product with user info, upvotes, and images - ordered by helpfulness"""
        ...
        rows = app.db.execute('''
            ...
            LEFT JOIN (
                ...
                GROUP BY review_id
            ) upvote_counts ON upvote_counts.review_id = r.review_id
        ...
        for row in rows:
            ...
            if user_id:
                review.user_has_upvoted = ReviewUpvote.has_user_upvoted(review.id, 'product', user_id)
            else:
                review.user_has_upvoted = False
            
            # Get images
            review.images = ReviewImage.get_by_review(review.id, 'product')
